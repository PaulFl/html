...
<head>
    ...
    <title>Trésorerie X3 - Consommation kro</title>
</head>
...
$kro_consomme = $_POST['kros_consommees'];
...
$reponse = $bdd->query('SELECT datetime, creancier, nb_kros, prix_kro from achats_kro order by datetime');

$prix_total = 0;

$kro_trouvees = 0;

while ($kro_trouvees < $kro_consomme) {
    $donnees = $reponse->fetch();
    $creancier = $donnees['creancier'];
    $kros_crancier = $donnees['nb_kros'];
    $prix_unitaire = $donnees['prix_kro'];
    if ($kros_crancier >= ($kro_consomme - $kro_trouvees)) {
        $kro_trouvees += ($kro_consomme - $kro_trouvees);
    }
}
